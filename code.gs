...
var PUBLIC_APP_URL_PROD = "https://script.google.com/macros/s/AKfycbyYcGWMk9jyaMFeQKSDHm2JOMrqOkBoyS5qh7t2YfdVamShAAtBA4S1UuhiyJWUrNpr/exec";
var PUBLIC_APP_URL_DEV = "https://script.google.com/macros/s/AKfycbyYcGWMk9jyaMFeQKSDHm2JOMrqOkBoyS5qh7t2YfdVamShAAtBA4S1UuhiyJWUrNpr/dev";
var USE_DEV_APP_URL = false; // true = test latest saved Apps Script code via /dev, false = public deployed /exec
var PUBLIC_APP_URL = USE_DEV_APP_URL ? PUBLIC_APP_URL_DEV : PUBLIC_APP_URL_PROD; // TODO: เปลี่ยนเป็น cloudflare เมื่อ setup redirect ?join= แล้ว
...
function getDashboardViewUrl() {
  ...
  if (!url) return '';
  return _withLineExternalParam_(url + (url.indexOf('?') === -1 ? '?' : '&') + 'view=dashboard');
}
...
function getEOCCallStatus() {
  ...
  var empty = { busy: false, count: 0, calls: [], callerName: '', callerRole: '', roomName: configuredRoom };
  if (!raw) return empty;
  try {
    var data = JSON.parse(raw);
    // backward compat: old format had {busy, callerName, callerRole, roomName}
    var calls = Array.isArray(data.calls) ? data.calls :
      (data.busy ? [{ roomName: data.roomName || configuredRoom, callerName: data.callerName || '', callerRole: data.callerRole || '', startedAt: data.startedAt || '' }] : []);
    var count = calls.length;
    var first = calls[0] || {};
    return { busy: count > 0, count: count, calls: calls, callerName: first.callerName || '', callerRole: first.callerRole || '', roomName: first.roomName || configuredRoom };
  } catch (e) {
...
// ============================================================
var DAILY_API_KEY = '97820b1944a771dcbd19477598a79259863acc51c5a101550c96c5b5068153c8';
var DAILY_DOMAIN  = 'the-1-ics.daily.co';

function _createDailyRoom_(roomName) {
  var payload = {
    name: roomName,
    properties: {
  ...
  try {
    var res = UrlFetchApp.fetch('https://api.daily.co/v1/rooms', {
      method: 'POST',
    ...
    return data.url || ('https://' + DAILY_DOMAIN + '/' + roomName);
  } catch(e) {
...
function setEOCCallBusy(callerName, callerRole, maxCalls) {
  maxCalls = (maxCalls && maxCalls > 0) ? maxCalls : 1;
  var lock = LockService.getScriptLock();
  lock.waitLock(5000);
  try {
    var current = getEOCCallStatus();
    if (current.count >= maxCalls) {
      var first = current.calls[0] || {};
      return { ok: false, full: true, count: current.count, maxCalls: maxCalls, callerName: first.callerName, callerRole: first.callerRole, roomName: first.roomName };
    }
    var roomName = _makeVideoRoomName_((callerName || 'CALL'));
    var newCall = { roomName: roomName, callerName: callerName || 'ไม่ระบุ', callerRole: callerRole || '-', startedAt: new Date().toISOString() };
    var updatedCalls = current.calls.concat([newCall]);
    PropertiesService.getScriptProperties().setProperty('EOC_CALL_STATUS', JSON.stringify({ calls: updatedCalls }));
    return { ok: true, count: updatedCalls.length, calls: updatedCalls, roomName: roomName, callerName: newCall.callerName, callerRole: newCall.callerRole, busy: true };
  } finally {
    lock.releaseLock();
  }
}

function clearEOCCallByRoom(roomName) {
  var lock = LockService.getScriptLock();
  lock.waitLock(5000);
  try {
    var current = getEOCCallStatus();
    var updated = current.calls.filter(function(c) { return c.roomName !== roomName; });
    if (updated.length === 0) {
      PropertiesService.getScriptProperties().deleteProperty('EOC_CALL_STATUS');
    } else {
      PropertiesService.getScriptProperties().setProperty('EOC_CALL_STATUS', JSON.stringify({ calls: updated }));
    }
    return { ok: true };
  } finally {
    lock.releaseLock();
...
function _clearDataRows_(sheetName, ss) {
  ss = ss || SpreadsheetApp.openById(SSID);
  var sheet = ss.getSheetByName(sheetName);
  if (!sheet) return;
  var lastRow = sheet.getLastRow();
  var lastCol = sheet.getLastColumn();
  // 🔧 ใช้ clearContent() แทน deleteRows() — deleteRows() จะชน Sheets API ที่ห้ามลบจนไม่เหลือแถวที่ไม่ตรึงไว้เลย (error: ลบทุกแถวที่ไม่ได้ตรึงไว้ไม่ได้)
...
function _resetIncidentOperationalData_() {
  // 🔧 เปิดสเปรดชีตครั้งเดียวแล้วใช้ซ้ำ แทนที่จะเปิดใหม่ทุกชีท (เดิมช้าตอนจบภารกิจ)
  var ss = SpreadsheetApp.openById(SSID);
  // ⚠️ Broadcast_Events ตั้งใจไม่ล้าง — เก็บไว้เป็นประวัติการเปิด/ปิดศูนย์ข้ามภารกิจ
  [
    "Triage_Data",
    "Incident_Logs",
    "Live_Locations",
    ...
    "Health_Units",
    "Evacuation_Points"
  ].forEach(function(name) { _clearDataRows_(name, ss); });
...
var TIER_CONFIG = {
  '1': {
    ...
    maxEvacPoints: 1,
    maxPhoneDirectory: 0,
    maxFieldMediaFromOSC: 5,
    maxSupportRequests: 5,
    features: {
      // 🟢 บทบาท ICS — Tier 1 เข้าได้ครบทุกกล่อง (ทุกคน ไม่จำกัดจำนวน)
      role_ic: true, role_oc: true,
      role_jic: true, role_liaison: true, role_specialist: true,
      role_logistics: true, role_planning: true,
      role_med: true, role_evac_point: true,
      // 🟢 งานพื้นฐานที่ทุกระดับต้องมี (casualty_report = ยอดผู้บาดเจ็บประมาณการจาก OC/ICP ถือเป็นข้อมูลพื้นฐาน)
      ic_dashboard: true, attendance: true, evacuation_point: true, casualty_report: true,
      // 🔒 เครื่องมือขั้นสูงในแต่ละ scene — ปลดล็อคที่ Tier 2
      dashboard_realtime: false,
      erg: false, broadcast: false, support_request: false,
      live_location: false, media_upload: false,
      ...
      exposure_log: false, role_navigation: false,
      chemical_info: false, sitrep: false, phone_directory: false,
      hospital_directory: false
    }
  },
  '2': {
    name: 'ตอบสนองขั้นสูง',
    ...
    maxShareViewers: 5,
    maxEvacPoints: -1,
    maxPhoneDirectory: 10,
    maxFieldMediaFromOSC: -1,
    maxSupportRequests: -1,
    features: {
      role_ic: true, role_oc: true,
      role_jic: true, role_liaison: true, role_specialist: true,
      role_logistics: true, role_planning: true,
      ...
      multi_incident: false, unified_command: false,
      eoc_video_call: true, multi_agency_zones: false,
      multi_hospital: false, decon_tracking: false, ambulance_tracking: false,
      mci: false, agency_master_sheet: false, line_multi_groups: false,
      device_binding: false, custom_expiry: false, api_integration: false,
      exposure_log: false, role_navigation: false, // exposure_log = Tier 3 เท่านั้น
      chemical_info: true, sitrep: true, phone_directory: true,
      hospital_directory: false
    }
  },
  '3': {
    ...
    features: {
      ...
      exposure_log: true, role_navigation: true,
      chemical_info: true, sitrep: true, phone_directory: true,
      hospital_directory: true
    }
  }
};
...
/* ============================================================
 * 📞 ทำเนียบเบอร์โทรหน่วยงานภายนอก (Phone Directory) — Liaison, Tier 2+
 *    - แอดมินเป็นผู้กรอก/จัดการ ตั้งค่าครั้งเดียวใช้ตลอด แก้ไข/เพิ่มได้ตลอด
 *    - จำกัดจำนวนตาม Tier: Tier 2 = 10 เบอร์ / Tier 3 = ไม่จำกัด
 * ============================================================ */
function _phoneDirKey_(agencyId) { return 'PHONE_DIR_' + String(agencyId || '').trim(); }

function getPhoneDirectory(agencyId) {
  agencyId = String(agencyId || '').trim();
  if (!checkTierFeature(agencyId, 'phone_directory')) return [];
  var props = PropertiesService.getScriptProperties();
  var raw = props.getProperty(_phoneDirKey_(agencyId));
  if (!raw) return [];
  try { return JSON.parse(raw) || []; } catch (e) { return []; }
}

function savePhoneDirectoryEntry(agencyId, entry, accessRole) {
  agencyId = String(agencyId || '').trim();
  if (String(accessRole || '').trim() !== 'admin') throw new Error('เฉพาะแอดมินเท่านั้นที่จัดการทำเนียบเบอร์โทรได้');
  _requireTierFeature_(agencyId, 'phone_directory', 'ทำเนียบเบอร์โทรหน่วยงานภายนอก (ระดับ 2+)');
  entry = entry || {};
  var name = String(entry.name || '').trim();
  var phone = String(entry.phone || '').trim();
  var category = String(entry.category || 'อื่นๆ').trim();
  if (!name || !phone) throw new Error('กรุณาระบุชื่อหน่วยงานและเบอร์โทร');
  var props = PropertiesService.getScriptProperties();
  var key = _phoneDirKey_(agencyId);
  var list = [];
  try { list = JSON.parse(props.getProperty(key) || '[]') || []; } catch (e) { list = []; }
  var id = String(entry.id || '').trim();
  var tier = getAgencyTier_(agencyId);
  var cfg = getTierConfig(tier);
  var limit = cfg ? cfg.maxPhoneDirectory : 10;
  if (id) {
    var idx = -1;
    for (var i = 0; i < list.length; i++) { if (list[i].id === id) { idx = i; break; } }
    if (idx === -1) throw new Error('ไม่พบรายการที่ต้องการแก้ไข');
    list[idx].name = name; list[idx].phone = phone; list[idx].category = category;
  } else {
    if (limit !== -1 && list.length >= limit) {
      throw new Error('เพิ่มเบอร์โทรได้สูงสุด ' + limit + ' รายการ สำหรับ Tier ' + tier + ' กรุณาอัปเกรดแพ็กเกจ');
    }
    id = Utilities.getUuid().replace(/-/g, '').substring(0, 12);
    list.push({ id: id, name: name, phone: phone, category: category, createdAt: Date.now() });
  }
  props.setProperty(key, JSON.stringify(list));
  return { ok: true, list: list };
}

function deletePhoneDirectoryEntry(agencyId, entryId, accessRole) {
  agencyId = String(agencyId || '').trim();
  if (String(accessRole || '').trim() !== 'admin') throw new Error('เฉพาะแอดมินเท่านั้นที่จัดการทำเนียบเบอร์โทรได้');
  var props = PropertiesService.getScriptProperties();
  var key = _phoneDirKey_(agencyId);
  var list = [];
  try { list = JSON.parse(props.getProperty(key) || '[]') || []; } catch (e) { list = []; }
  list = list.filter(function (e) { return e.id !== entryId; });
  props.setProperty(key, JSON.stringify(list));
  return { ok: true, list: list };
}

/* ============================================================
 * 🏥 ทำเนียบโรงพยาบาลในพื้นที่ (Hospital Directory) — MED, Tier 3
 *    - รพ. ไหนรับผู้ป่วยประเภทใดได้บ้าง — แอดมินเป็นผู้จัดการ
 * ============================================================ */
function _hospitalDirKey_(agencyId) { return 'HOSPITAL_DIR_' + String(agencyId || '').trim(); }

function getHospitalDirectory(agencyId) {
  agencyId = String(agencyId || '').trim();
  if (!checkTierFeature(agencyId, 'hospital_directory')) return [];
  var props = PropertiesService.getScriptProperties();
  var raw = props.getProperty(_hospitalDirKey_(agencyId));
  if (!raw) return [];
  try { return JSON.parse(raw) || []; } catch (e) { return []; }
}

function saveHospitalDirectoryEntry(agencyId, entry, accessRole) {
  agencyId = String(agencyId || '').trim();
  if (String(accessRole || '').trim() !== 'admin') throw new Error('เฉพาะแอดมินเท่านั้นที่จัดการทำเนียบโรงพยาบาลได้');
  _requireTierFeature_(agencyId, 'hospital_directory', 'ทำเนียบโรงพยาบาลในพื้นที่ (ระดับ 3+)');
  entry = entry || {};
  var name = String(entry.name || '').trim();
  var acceptTypes = String(entry.acceptTypes || '').trim();
  if (!name) throw new Error('กรุณาระบุชื่อโรงพยาบาล');
  var props = PropertiesService.getScriptProperties();
  var key = _hospitalDirKey_(agencyId);
  var list = [];
  try { list = JSON.parse(props.getProperty(key) || '[]') || []; } catch (e) { list = []; }
  var id = String(entry.id || '').trim();
  if (id) {
    ...
    for (var i = 0; i < list.length; i++) { if (list[i].id === id) { idx = i; break; } }
    if (idx === -1) throw new Error('ไม่พบรายการที่ต้องการแก้ไข');
    list[idx].name = name; list[idx].acceptTypes = acceptTypes; list[idx].phone = String(entry.phone || '').trim();
  } else {
    id = Utilities.getUuid().replace(/-/g, '').substring(0, 12);
    list.push({ id: id, name: name, acceptTypes: acceptTypes, phone: String(entry.phone || '').trim(), createdAt: Date.now() });
  }
  props.setProperty(key, JSON.stringify(list));
  return { ok: true, list: list };
}

function deleteHospitalDirectoryEntry(agencyId, entryId, accessRole) {
  agencyId = String(agencyId || '').trim();
  if (String(accessRole || '').trim() !== 'admin') throw new Error('เฉพาะแอดมินเท่านั้นที่จัดการทำเนียบโรงพยาบาลได้');
  var props = PropertiesService.getScriptProperties();
  var key = _hospitalDirKey_(agencyId);
...
// 📱 LINE in-app browser เปิด GAS web app แล้วมักพัง (cookie/sandbox)
// openExternalBrowser=1 เป็น param ทางการของ LINE — สั่งให้เด้งไปเปิด Chrome/Safari อัตโนมัติ
// แอปอื่น/เบราว์เซอร์ปกติจะเมิน param นี้ ไม่มีผลข้างเคียง
function _withLineExternalParam_(url) {
  if (!url) return url;
...
function _buildJoinUrl_(token) {
  ...
  if (!url || !token) return '';
  return _withLineExternalParam_(url + (url.indexOf('?') === -1 ? '?' : '&') + 'join=' + encodeURIComponent(token));
}
...
function searchPlaces(query) {
  ...
  var requests = [];
  // 🔧 Maps.newGeocoder().geocode() เป็น sync call ทีละตัว (ช้ามาก ~1-2 วิ/ครั้ง)
  // เดิมยิง 10 ครั้งต่อการค้นหา = รอเป็น 10+ วินาที จึงลดเหลือ 2 (คำค้นเดิม + คำขยายอันดับแรก) — Longdo (ยิงขนานผ่าน fetchAll) ครอบคลุมผลลัพธ์ส่วนใหญ่อยู่แล้ว
  var googleQueries = queries.slice(0, 2);
  var googleLists = [];
...
/**
 * 🔧 ตรวจสอบรหัสผ่าน admin ของหน่วยงาน สำหรับเมนู "ตั้งค่าข้อมูลหน่วยงาน" ก่อนเข้าสู่ระบบ
 * อ่านอย่างเดียว — ไม่นับจำนวน login / ไม่ผูก device / ไม่อัปเดต LastLogin (ต่างจาก checkAgencyLogin_)
 * เพราะเป็นแค่การ "แอบดู" สิทธิ์และ tier เพื่อเปิดเมนูตั้งค่าทำเนียบ ไม่ใช่การเข้าสู่ระบบจัดการเหตุฉุกเฉินจริง
 */
function checkAgencySetupAccess(inputPass) {
  var pass = String(inputPass || '');
  if (!pass) return { ok: false, reason: 'invalid' };
  var sheet = ensureAgenciesSchema_();
  var lastRow = sheet.getLastRow();
  if (lastRow < 2) return { ok: false, reason: 'invalid' };
  var values = sheet.getRange(2, 1, lastRow - 1, 12).getValues();
  for (var i = 0; i < values.length; i++) {
    var row = values[i];
    if (String(row[2] || '') !== pass) continue;
    var status = String(row[5] || 'ACTIVE').trim().toUpperCase();
    if (status !== 'ACTIVE') return { ok: false, reason: 'disabled' };
    if (_isAgencyExpired_(row[6])) return { ok: false, reason: 'expired' };
    if (_normalizeAccessRole_(row[3]) !== 'admin') return { ok: false, reason: 'not_admin' };
    var tier = _normalizeTier_(row[11]);
    ...
    return {
      ...
      tier: tier,
      features: {
        phone_directory: !!(cfg && cfg.features && cfg.features.phone_directory),
        hospital_directory: !!(cfg && cfg.features && cfg.features.hospital_directory)
      }
    };
  }
 ...
 */
// 🔧 เขียนค่า Config หลายคีย์รวดเดียวด้วย setValues() เดียว แทนการ setValue() ทีละเซลล์ (ลดเวลาเปิดเหตุจาก ~30 ครั้ง เหลือ 1 ครั้ง)
function _setConfigBatch_(sheet, data, updates) {
  ...
  for (var key in updates) {
    ...
    if (keyRow.hasOwnProperty(key)) {
      values[keyRow[key]][1] = updates[key];
      data[keyRow[key]][1] = updates[key];
    } else {
      var row = [key, updates[key]];
      values.push(row);
      data.push(row);
      keyRow[key] = values.length - 1;
    }
  }
...
function activateEmergency(evtName, evtLoc, evtCoords, evtPlan, evtLevel, evtEOC, commanderName, accessRole, commanderPosition, windDirectionDeg, windSpeedMs, windMode, agencyId, eocCoords) {
  ...
  var updates = {
    ...
    IncidentName: evtName,
    IncidentLocation: evtLoc,
    IncidentCoords: evtCoords,
    EOCLocation: evtEOC,
    EOCCoords: checkTierFeature(agencyId, 'share_link') ? String(eocCoords || '').trim() : '',
    CommanderName: commanderName || '',
    IncidentCommander: commanderName || '',
    CommanderPosition: commanderPosition || '',
    RegisteredICName: '',
    RegisteredICPosition: '',
    RegisteredICPhone: '',
    RegisteredICTime: '',
    StartTime: new Date(),
    Res_Ambulance: 0,
  ...
  if (windDeg !== '' && !isNaN(windDeg) && windDeg >= 0 && windDeg < 360 && windSpeed !== '' && !isNaN(windSpeed) && windSpeed >= 0) {
    updates.Wind_Direction_Deg = windDeg;
    updates.Wind_Speed_MS = windSpeed;
    updates.Wind_Source = autoWind ? 'Auto' : 'Admin';
    updates.Wind_UpdatedBy = commanderName || 'Admin';
    updates.Wind_UpdatedAt = new Date();
  } else {
    updates.Wind_Direction_Deg = '';
    updates.Wind_Speed_MS = '';
    updates.Wind_Source = '';
    updates.Wind_UpdatedBy = '';
    updates.Wind_UpdatedAt = '';
  }
  updates.Wind_Pending_Direction_Deg = '';
  updates.Wind_Pending_Speed_MS = '';
  updates.Wind_Pending_Source = '';
  updates.Wind_Pending_UpdatedBy = '';
  updates.Wind_Pending_UpdatedAt = '';

  var fullPlan = evtPlan;
  if (evtLevel && evtLevel !== '-') fullPlan += " (" + evtLevel + ")";
  updates.PlanInfo = fullPlan;
  var videoRoomName = _makeVideoRoomName_(evtName);
  updates.VideoRoomName = videoRoomName;
...
function setEOCCoords(lat, lng, accessRole, agencyId) {
  _requireAdmin(accessRole);
  _requireTierFeature_(agencyId, 'share_link', 'ตั้งพิกัดที่ตั้ง EOC (ระดับ 2+)');
...
function deactivateEmergency(commanderName, accessRole, agencyId) {
  ...
  _setConfig(sheet, data, 'ERG_Iso_M', '');
  _setConfig(sheet, data, 'ERG_Prot_Day_M', '');
  _setConfig(sheet, data, 'ERG_Prot_Night_M', '');
  _setConfig(sheet, data, 'ERG_UpdatedAt', '');
  _clearEmergencyJoinLink_(agencyId, sheet, data);

  setEOCCallFree();
  // 🔧 ล้างข้อมูลปฏิบัติการ + log ปิดศูนย์ ต้องอยู่สเปรดชีตเดียวกับ Config (mainSSID)
  // ไม่ใช้ SSID ตรงๆ เพราะ _useAgencySpreadsheetForRequest_ ด้านบนอาจสลับ SSID ไปยังชีทอื่นแล้ว
  SSID = mainSSID;
  _resetIncidentOperationalData_();
...
function getERGState(agencyId, sheetId, joinToken) {
  // 🔧 route ไปยัง sheet ที่ถูกต้องก่อน (สำคัญสำหรับ Join Token session — ถ้าไม่ route จะอ่าน SSID default แทน agency sheet)
...
function submitEmergencyAttendance(name, role, location, phone, roleCode, agencyId) {
  ...
  if (agencyId && roleCode) {
    var roleFeatureMap = {
      'IC':'role_ic','OSC':'role_oc','OC':'role_oc',
      'JIC':'role_jic','Liaison':'role_liaison','Specialist':'role_specialist',
      'Logistics':'role_logistics','Planning':'role_planning',
    ...
    if (effId) {
      ...
      if (cfg && cfg.maxZones > 0) {
        ...
        if (current >= cfg.maxZones) {
          var zoneDesc = cfg.maxZones === 3 ? '3 จุด (Command Post 1 + จุดปฏิบัติการ 2)' : cfg.maxZones === 5 ? '5 จุด (Command Post 1 + จุดปฏิบัติการ 4)' : cfg.maxZones === 10 ? '10 จุด (Command Post 1 + จุดปฏิบัติการ 9)' : cfg.maxZones + ' จุด';
...
// ============================================================
// 📤 Direct Drive Upload (bypass GAS payload limit)
// ============================================================
function _isTier1OCImageUploadAllowed_(agencyId, source, mimeType) {
  var tier = String(getAgencyTier_(agencyId || '') || '');
  if (tier !== '1') return false;

  var raw = String(source || '');
  var src = raw.toUpperCase();
  var isOC = src === 'OC' || src === 'OSC' || src.indexOf('OC/ICP') !== -1;
  // 🎚️ Tier 1: MED (สาธารณสุข/1669) และ EVAC (จุดอพยพ) ส่งรูปได้เหมือน OC/ICP
  var isMedEvac = raw.indexOf('สาธารณสุข') !== -1 || src === 'MED' || raw.indexOf('จุดอพยพ') !== -1 || src === 'EVAC';
  var isImage = String(mimeType || '').indexOf('image/') === 0;

  return (isOC || isMedEvac) && isImage;
}

// 🎚️ Tier 1: โควต้ารูปสะสมต่อ 1 เหตุการณ์ — OC/ICP, MED, EVAC ส่งได้บทบาทละ 5 รูป
// (Media_Reports ถูกล้างตอนปิดเหตุ → นับจากชีตได้ตรงตามเหตุการณ์ปัจจุบัน)
...
function _assertTier1MediaQuota_(agencyId, source, addCount) {
  if (!agencyId || checkTierFeature(agencyId, 'media_upload')) return;
  var bucket = _tier1MediaBucket_(source);
  if (!bucket) return;
  var cfg = getTierConfig(getAgencyTier_(agencyId)) || {};
  var limit = cfg.maxFieldMediaFromOSC;
  ...
  if (lastRow > 1) {
    ...
    for (var i = 0; i < values.length; i++) {
      if (_tier1MediaBucket_(values[i][0]) === bucket) used++;
    }
  }
  if (used + (addCount || 1) > limit) {
    throw new Error('Tier 1: ' + bucket + ' ส่งรูปได้สูงสุด ' + limit + ' รูปต่อ 1 เหตุการณ์ (ส่งไปแล้ว ' + used + ' รูป)');
  }
}
function getFieldMediaUploadToken(agencyId, source, mimeType) {
  if (agencyId && !checkTierFeature(agencyId, 'media_upload') && !_isTier1OCImageUploadAllowed_(agencyId, source, mimeType)) {
    throw new Error('Tier 1 (OC/ICP, MED, จุดอพยพ) ส่งได้เฉพาะรูปภาพ ไม่รองรับวิดีโอ/เสียง/ไฟล์อื่น');
  }
  _assertTier1MediaQuota_(agencyId, source, 1);
  var folder = getOrCreateFieldMediaFolder_();
  return { token: ScriptApp.getOAuthToken(), folderId: folder.getId() };
}

function registerFieldMediaFile(fileId, source, reporter, fileName, mimeType, note, agencyId) {
  // 🎚️ Tier guard — ด่านสุดท้ายของ direct Drive upload (token ขอไปก่อนหน้าแล้ว แต่บังคับซ้ำกันโกง)
  if (agencyId && !checkTierFeature(agencyId, 'media_upload') && !_isTier1OCImageUploadAllowed_(agencyId, source, mimeType)) {
    throw new Error('Tier 1 (OC/ICP, MED, จุดอพยพ) ส่งได้เฉพาะรูปภาพ ไม่รองรับวิดีโอ/เสียง/ไฟล์อื่น');
  }
  _assertTier1MediaQuota_(agencyId, source, 1);
  ...
  sheet.appendRow([timestamp, source || '-', reporter || '-', fileName || file.getName(), mimeType || '', file.getUrl(), fileId, note || '']);
  addCommanderLog('แนบสื่อจากพื้นที่: ' + (source || '-') + ' / ' + (fileName || '-'), reporter || source || 'Field');
  return {
    time: _formatDateSafe_(timestamp, 'dd/MM HH:mm'), source: source || '-', reporter: reporter || '-',
    fileName: fileName || file.getName(), mimeType: mimeType || '', url: file.getUrl(), fileId: fileId,
    directUrl: 'https://drive.google.com/uc?export=view&id=' + fileId,
    thumbUrl: 'https://drive.google.com/thumbnail?id=' + fileId + '&sz=w1000',
    previewUrl: 'https://drive.google.com/file/d/' + fileId + '/preview', note: note || ''
  };
}
...
function submitFieldCasualtyReport(totalEstimate, stillInArea, evacuatedOrSent, note, loggedBy, agencyId) {
  // 🎚️ Tier guard
  if (agencyId) _requireTierFeature_(agencyId, 'casualty_report', 'ยอดผู้บาดเจ็บประมาณการ (ระดับ 2+)');
  var ss = SpreadsheetApp.openById(SSID);
...
function submitSupportRequest(requestType, detail, loggedBy, agencyId) {
  // 🎚️ Tier guard — Tier 1 (OC/ICP) ขอสนับสนุนได้แบบจำกัดจำนวนต่อ 1 เหตุการณ์ (Support_Request ถูกล้างตอนปิดเหตุ)
  if (agencyId && !checkTierFeature(agencyId, 'support_request')) {
    var maxReq = (getTierConfig(getAgencyTier_(agencyId)) || {}).maxSupportRequests;
    if (!maxReq) _requireTierFeature_(agencyId, 'support_request', 'ระบบขอสนับสนุน');
    if (maxReq > 0) {
      var qSheet = ensureSupportRequestSchema_();
      var used = Math.max(0, qSheet.getLastRow() - 1);
      if (used >= maxReq) {
        throw new Error('Tier 1: ขอสนับสนุนได้สูงสุด ' + maxReq + ' คำขอต่อ 1 เหตุการณ์ (ส่งไปแล้ว ' + used + ' คำขอ)');
      }
    }
  }
...
function createDashboardViewToken(agencyId) {
  agencyId = String(agencyId || '').trim();
  _requireTierFeature_(agencyId, 'share_link', 'แชร์ลิงก์ดูแดชบอร์ดอย่างเดียว');
  var tier = getAgencyTier_(agencyId);
  // 🔒 จำกัดจำนวนลิงก์ดูอย่างเดียวที่ใช้งานพร้อมกันได้ตาม Tier: 1=1, 2=5, 3=10
  var VIEW_LINK_LIMIT = {'2': 5, '3': 10 };
  var maxViewers = VIEW_LINK_LIMIT[tier] || 1;
  var props = PropertiesService.getScriptProperties();
  var now = Date.now();
  var EXPIRY_MS = 8 * 60 * 60 * 1000;
  if (agencyId) {
    var allProps = props.getProperties();
    var activeCount = 0;
    for (var key in allProps) {
      if (key.indexOf('DASH_VIEW_TOKEN_') !== 0) continue;
      try {
        var d = JSON.parse(allProps[key]);
        if (d.agencyId === agencyId && (now - d.createdAt) <= EXPIRY_MS) {
          activeCount++;
        }
      } catch (e) {}
    }
    if (activeCount >= maxViewers) {
      throw new Error('สร้างลิงก์ดูอย่างเดียวได้สูงสุด ' + maxViewers + ' ลิงก์พร้อมกัน (Tier ' + tier + ') — กรุณารอให้ลิงก์เก่าหมดอายุ (8 ชม.) หรือใช้ลิงก์เดิมที่ยังไม่หมดอายุ');
    }
  }
  ...
  var url = (PUBLIC_APP_URL || ScriptApp.getService().getUrl());
  return _withLineExternalParam_(url + (url.indexOf('?') === -1 ? '?' : '&') + 'vtoken=' + token);
}
...
var record = data.agencyId ? _getAgencyRecordBy_('AgencyId', data.agencyId) : null;
return {
  valid: true,
  agencyId: data.agencyId || '',
  sheetId: record ? (record.currentSheetId || record.sheetId || '') : '',
  tier: record ? record.tier : '',
  tierConfig: record ? record.tierConfig : null
};
  } catch(e) {
